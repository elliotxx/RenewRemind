...
### 扩展区
def Comic_banshoushaonian():
    global subject,content,isRenew,renew_dict
    key = '扳手少年'
    url = 'http://ac.qq.com/Comic/ComicInfo/id/520794'
    host = 'http://ac.qq.com'
    html = get_html(url,timeout).decode('utf-8')    # 获得页面源码

    # 提示信息
    print '正在检查【%s】的更新状态……'%(key)

    # 解析源码
    pattern = re.compile(r'<a class="works-ft-new" href="(.*?)">(.*?)</a><span.*?>(.*?)</span>',re.S)
    ...
    if cur != last or last==None:
        ...
        subject += '%s '%(key)
        content += '【%s】已经更新到【%s】，戳这里看详情：%s<br/>'%(key,cur,href.encode('utf8'))
    else:
        # 没有更新
        print '【%s】没有更新'%(key)

def Novel_douzhankuangchao():
    global subject,content,isRenew,renew_dict
    key = '斗战狂潮'
    url = 'http://www.qidian.com/Book/1003694333.aspx'
    ...
    # 解析源码
    pattern = re.compile(r'</b><a class="blue" href=".*?" data-eid="qd_G19" data-cid=".*?" title=".*?" target="_blank">(.*?)</a><i>.*?</i><em class="time">(.*?)</em>',re.S)
    items = re.findall(pattern,html)
    ...
    if cur != last or last==None:
        ...
        print '【%s】有更新，发送邮件……'%(key)
        show_url = 'http://www.booktxt.net/2_2322/'     # 顶点小说网的链接
        subject += '%s '%(key)
        content += '【%s】已经更新到【%s】，戳这里看详情：%s<br/>'%(key,cur,show_url)
    else:
        # 没有更新
        print '【%s】没有更新'%(key)
...
def main():
    ...
    # 检查所有更新，并输出提示信息
    Comic_banshoushaonian()	# 漫画：扳手少年
    Novel_douzhankuangchao()	# 小说：斗战狂潮
